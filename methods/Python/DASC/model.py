...
class Projection(Layer):
	...
	def call(self, z):
		...
		# U = utils.u_regularize(self.U)
		self.U = tf.cast(self.U, 'float64')
		z = tf.cast(z, 'float64')
		z = K.dot(K.transpose(self.U), z)
		# z = np.dot(np.transpose(self.U.numpy()), z.numpy())
		# z = K.relu(z)
		z = K.dot(self.U, z)
		# z = K.relu(z)
		return z
...
class ConvAE(Model):
	def __init__(self, input_shape=(None, 8, 8, 1), batch_size=None):
		super(ConvAE, self).__init__()

		# self.input_shape = input_shape # [32, 32, 1] i.e. 32x32x1 单通道图像
		self.batch_size = batch_size

		self.input_shape_0 = input_shape[1:][0]
		self.input_shape_1 = input_shape[1:][1]

		self.output_size = int(self.input_shape_0/2 * self.input_shape_1/2 * 15)
		...
		self.encoder = Sequential([
			layers.Conv2D(15, kernel_size=3,
						  ...
						  strides=2,
						  padding='SAME',
						  kernel_initializer = tf.keras.initializers.glorot_uniform(),#tf.keras.initializers.GlorotUniform(),
						  kernel_regularizer=tf.keras.regularizers.l1()),
			layers.Reshape((-1, self.output_size), trainable=False)
		])
		...
		self.decoder = Sequential([
			layers.Reshape((int(self.input_shape_0/2), int(self.input_shape_1/2), 15), trainable=False),
	...
	def call(self, x):
		z = self.encoder(x)
		z = tf.reshape(z, [self.batch_size, self.output_size])	# 整个batch一起训练
		self.z_conv = z
		# print(z)
		z = self.self_expressive(z)
		self.z_se = z									# self_expressive_z = theta*z
		# print("z_se:", z.shape)
		z = tf.reshape(z, [self.batch_size, 1, self.output_size])
		x = self.decoder(z)
...
class DASC(object):
	def __init__(self, model, input_shape, batch_size=1440, kcluster=10, r=30):
