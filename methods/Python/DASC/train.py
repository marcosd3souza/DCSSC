...
from .model import ConvAE, Mnist_ConvAE
...
def train(name, train_data, batch_size=72, input_shape=[8, 8, 1], epoch_num=10, pre_train_epoch=10,
    ...
    if name == 'MNIST':
        dasc = DASC(Mnist_ConvAE, input_shape=inputs, batch_size=batch_size, kcluster=10)
    else:
        dasc = DASC(ConvAE, input_shape=inputs, batch_size=batch_size, kcluster=20)
    ...
    # # d_var = dasc._U
    # d_optim = tf.optimizers.Adam(learning_rate=d_lr)
    # g_optim = tf.optimizers.Adam(learning_rate=g_lr)
    ...
    #                     r2 = loss.r2(d_var)
    #                     l_d = l_d + r1 + r2
    #                     # print("\tLoss_D: loss={}\tR1={}\tR2={}".format(str(float(l_d)), str(float(loss.r1(d_var))),
    #                     #                                                str(float(loss.r2(d_var)))))
    ...
    #             rec_loss, reconst_loss, self_expr_loss, penalty = loss.ae_loss(x_batch, x_reconst, z_conv, z_se,
    #                                                                            theta)
    ...
    #             # sc = SpectralClustering(n_clusters=20, eigen_solver='arpack', affinity='precomputed', n_init=100,
    ...
    # return theta
    Z = 0.5 * (np.abs(theta) + np.abs(theta.T))
    print(Z.shape)
    return Z
