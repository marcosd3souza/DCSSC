...
from .post_clustering import spectral_clustering, thrC, post_proC

from .post_clustering import acc as ACC
...
def train(model,  # type: DSCNet
          x, epochs, lr=1e-3, weight_coef=1.0, weight_selfExp=150, device='cuda', alpha=0.04, ro=8, show=10):
...
def evaluate(C, y):
    if isinstance(y, torch.Tensor):
        y = y.to('cpu').numpy()
    K = len(np.unique(y))
    y_pred, Z = spectral_clustering(C, K, dim_subspace, alpha, ro)

    acc, nmi, ari = ACC(y, y_pred), NMI(y, y_pred), ARI(y, y_pred)
    print('acc=%.4f, nmi=%.4f, ari=%.4f', acc, nmi, ari)

    return Z, acc, nmi, ari


def dsc_net_exec(X_img):
    num_sample = X_img.shape[0]
    # X = X.reshape((-1, 1, 8, 8))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--db', default='simulated',
                        choices=['simulated', 'coil20', 'coil100', 'orl', 'reuters10k', 'stl'])
    ...
    C = train(dscnet, x, epochs, weight_coef=weight_coef, weight_selfExp=weight_selfExp, show=args.show_freq)
    Z, acc, nmi, ari = evaluate(C, y)
