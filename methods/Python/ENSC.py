...
class SelfRepresentation(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.representation_matrix_ = self._self_representation(X)
        ...
        cm = metrics.confusion_matrix(y, self.y_predict)
        ...
        nmi = metrics.normalized_mutual_info_score(y, self.y_predict)
        ari = metrics.adjusted_rand_score(y, self.y_predict)

        print(f'acc: {acc}')
        print(f'nmi: {nmi}')
        print(f'ari: {ari}')

        plt.imshow(self.representation_matrix_.toarray())
        plt.show()
    ...
    def fit_self_representation(self, X, y=None):
        ...
        Z = self._self_representation(X).toarray()
        self.timer_self_representation_ = time.time() - time_base
        Z = 0.5 * (np.absolute(Z) + np.absolute(Z.T))
    ...
    def _representation_to_affinity(self):
        ...
        if self.affinity == 'symmetrize':
            ...
            neighbors_graph = kneighbors_graph(normalized_representation_matrix_, 3, mode='connectivity', include_self=False)
    ...
    def _spectral_clustering(self):
        ...
        _, self.y_predict, _ = cluster.k_means(embedding, self.n_clusters,
                                             random_state=random_state, n_init=self.n_init)

    def _active_support_elastic_net(self, X, y, alpha, tau=1.0, algorithm='lasso_lars', support_init='knn',
    ...
    def _self_representation(self, X, gamma=50.0, gamma_nz=True, tau=1.0, algorithm='lasso_lars',
        ...
        #        print('SSC: sparse coding finished {i} in {n_samples}'.format(i=i, n_samples=n_samples))
            ...
            if algorithm in ('lasso_lars', 'lasso_cd', 'spams'):
                ...
                if active_support == True:
                    c = self._active_support_elastic_net(X, y, alpha, tau, algorithm, **active_support_params)
                else:
                    # if algorithm == 'spams':
                    ...
                    # else:
                    c = sparse_encode(X, y, algorithm=algorithm, alpha=alpha)[0]
            else:
...
if __name__ == "__main__":
    k = 30
    X, y = make_blobs(n_samples=120, cluster_std=10.0, centers=k, n_features=1024, random_state=0)
    SelfRepresentation(n_clusters=k).fit(X, y)
