...
def SparseCoefRecovery(Xp, cst=0, Opt='Lasso', lmbda=0.001):
    lmbda = cvx.Parameter(nonneg=True, value=lmbda)
    D, N = Xp.shape
    c = cvx.Variable(N - 1)
    CMat = np.zeros([N, N])
    for i in range(0, N):
        ...
        if cst == 1:
            if Opt == 'Lasso':
                # c = cvx.Variable(N - 1)
                term1 = cvx.norm(c, 1)
                term2 = cvx.norm(Y * c - y)
                obj = cvx.Minimize(term1 + lmbda @ term2)
            ...
            if Opt == 'Lasso':
                ...
                c = cvx.Variable(N - 1 + D)
                obj = cvx.Minimize(cvx.norm(c, 1))
                constraint = [Constraint([np.concatenate((Y, np.identity(D)), axis=1) * c == y])]
                prob = cvx.Problem(obj, constraint)
                prob.solve()
